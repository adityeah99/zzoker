...
export default function PlaylistClient({ playlist }: Props) {
  const { playSong, toggleShuffle } = usePlayer();
  const { showToast } = useToast();
  const songs = playlist.songs || [];
  const artwork = getImageUrl(playlist.image, '500x500');
    ...
    <div>
      ...
      <div className="relative">
        <div className="absolute inset-0 overflow-hidden h-80">
          <Image src={artwork} alt={playlist.name} fill className="object-cover blur-2xl opacity-30 scale-110" sizes="100vw" />
        ...
        <div className="relative px-6 pt-8 pb-6 flex flex-col sm:flex-row items-start sm:items-end gap-6">
          <div className="relative w-48 h-48 md:w-56 md:h-56 rounded-2xl overflow-hidden shadow-2xl shrink-0">
            <Image src={artwork} alt={playlist.name} fill className="object-cover" sizes="224px" />
          </div>

          <div className="pb-2">
            <p className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-2">Playlist</p>
            <h1 className="text-white text-3xl md:text-4xl font-bold leading-tight mb-2">{playlist.name}</h1>
            {playlist.description && (
              <p className="text-white/50 text-sm mb-3 max-w-md line-clamp-2">{playlist.description}</p>
            )}
            <div className="flex items-center gap-2 mb-4">
              {playlist.songCount && <span className="text-white/40 text-sm">{playlist.songCount} songs</span>}
              {playlist.fanCount && <span className="text-white/40 text-sm">• {playlist.fanCount.toLocaleString()} followers</span>}
            </div>
      ...
      <div className="px-2 md:px-4 mt-4">
        <div className="flex items-center gap-3 px-4 py-2 border-b border-white/5 mb-2">
          ...
          <span className="hidden md:block text-xs text-white/30 w-40">Album</span>
