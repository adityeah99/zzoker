...
// Hoisted outside parent — stable component reference, no remount on every render
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <button
        type="button"
        onClick={onApply}
        className="mt-4 w-full bg-red-500 hover:bg-red-600 active:bg-red-700 text-white font-semibold py-3 rounded-xl transition-colors text-sm"
      >
        Done
      </button>
    </div>
...
export default function LanguageSelector() {
  ...
  const apply = () => {
    ...
    setLanguages(saved);
    const names = saved.map((l) => LANGUAGE_META[l].label).join(', ');
    ...
    <div ref={ref} className="relative">
      ...
      {!isMobile && open && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-[#1c1c1e] border border-white/10 rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
          <LanguagePanel draft={draft} onToggle={toggle} onApply={apply} onClose={close} />
        </div>
      )}

      {/* Mobile bottom sheet — always mounted, toggled via CSS so animation works */}
      {isMobile && (
        <>
          {/* Backdrop */}
          <div
